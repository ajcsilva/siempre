...
curl_setopt(
  $ch,
  CURLOPT_URL,
  'https://api.github.com/repos/roots/trellis/releases',
);
...
copy(
  '.radicle-setup/trellis/build-after.yml',
  'trellis/deploy-hooks/build-after.yml',
);
copy(
  '.radicle-setup/trellis/build-before.yml',
...
$localIps = ['hosts/development', 'vagrant.default.yml'];
...
foreach ($wordpressSitesAndVaults as $file) {
  ...
  if ($file === 'group_vars/development/wordpress_sites.yml') {
    $content = str_replace(
      'example.test',
      str_replace('.' . pathinfo($domain, PATHINFO_EXTENSION), '', $domain) .
        '.test',
      $content,
    );
  }
