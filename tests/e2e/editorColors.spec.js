...
test('editor color indigo-100 exists', async ({ page }) => {
  ...
  await page.click(
    '[aria-label="Blocks"] button.editor-block-list-item-paragraph',
  );
  await page.fill('.editor-styles-wrapper p', 'Example text');
  await page.click(
    'button.block-editor-panel-color-gradient-settings__dropdown div[title="Text"]',
  );
  await page.click('[aria-label="Color: Indigo 100"]');
  const paragraph = await page.$('.editor-styles-wrapper p');
  expect(await paragraph?.getAttribute('style')).toContain(
