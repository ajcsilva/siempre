...
class ThemeServiceProvider extends SageServiceProvider {
  ...
  public function register() {
    ...
    add_action(
      ...
      function (): void {
        Collection::make(config('theme.support'))
          ->map(
            fn($params, $feature) => is_array($params)
              ? [$feature, $params]
              : [$params],
          )
    ...
    add_action('widgets_init', function (): void {
      Collection::make(config('theme.sidebar.register'))->map(
        fn($instance) => register_sidebar(
    ...
    /**
     * Disables the editor for templates
     * and pages that don't need it
     */
